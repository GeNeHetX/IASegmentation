...
def classNames =['Background', 'TISSUE FOR CLASSIF']   // names of classes of interest (simply add more values to list to add more classes)
double downsample = 2  // which downsampling level to use -> how much to downsample the patches
double glassThreshold = 22  // which threshold to use for tissue detection (lower value => more tissue included in mask)
...
def dir1 = new File(pathOutput + "/image");
if (!dir1.isDirectory())
    dir1.mkdir()
    
def dir2 = new File(pathOutput + "/mask");
if (!dir2.isDirectory())
    dir2.mkdir()
...
listOfFiles.each { tile ->
    ...
    if (amountTissue < percentageThreshold) {
        ...
        tile.renameTo(pathOutput + "/image/" + tile.getName())
        currLabelPatch.renameTo(new File(pathOutput + "/mask/" + tile.getName().split(imageExtension)[0] + ".png"))
    }
}
